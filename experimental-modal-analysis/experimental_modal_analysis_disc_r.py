...
# Import data
axis = 'r'
filename = 'experimental_modal_analysis_disc_r'
title = 'Axial oscillation of disc SMB Rotor'
m = 1.526 # Mass of rotor in kg
...
plt.plot(t_one[:int(N_one)], y_one[:int(N_one)],'-o',linewidth=0.8, color = 'k', markersize=1.2, markerfacecolor='darkblue',markeredgecolor='black')
...
# t_in = t_one.reshape((-1, 1))
# model.fit(t_in, y_one)

# plt.plot(t_one, y_one)
# plt.plot(t_one,model.predict(t_in),'r',label="Linear regression: " + str(round(model.coef_[0],3)) + " t + " + str(round(model.intercept_,3)))
# plt.legend()
# plt.xlabel('time [s]')
# plt.ylabel('displacement [mm]')
# plt.savefig('displacement.png')
# y_one = y_one - model.predict(t_in)
...
def func(tt, g1, g2, phi1, phi2, A1, A2):
    # A1 = np.max(y_one)
    f1 = f_fft[0]
    f2 = f_fft[1]
...
p, pcov = curve_fit(func, t_one[:int(N_one/2)], y_one[:int(N_one/2)])
print(p)

f_fitted = f_fft[0]
# gamma1_fitted, phi1_fitted = p
gamma1_fitted, gamma2_fitted, phi1_fitted, phi2_fitted, A1_fitted, A2_fitted = p
...
plt.figure(figsize=(6,4))
plt.grid(color='grey', linestyle='-', linewidth=0.2)
plt.plot(t_one[:int(N_one/2)], y_one[:int(N_one/2)],'-.x',linewidth=0.8, color = 'lightgrey', markersize=1.2, markerfacecolor='darkblue',markeredgecolor='black')
plt.plot(t_one[:int(N_one/2)], func(t_one[:int(N_one/2)], *p), '-',linewidth=1.2, color = 'royalblue', markersize=1, markerfacecolor='darkblue', markeredgecolor='k')
plt.xlabel('$t$ [s]')
plt.ylabel('$' + axis + '$ [mm]')
plt.title(title)
