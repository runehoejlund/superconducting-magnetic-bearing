...
# Import data
axis = 'z'
filename = 'experimental_modal_analysis_disc_z'
...
plt.plot(t_one[:int(N_one/2)], y_one[:int(N_one/2)],'-o',linewidth=0.8, color = 'k', markersize=1.2, markerfacecolor='darkblue',markeredgecolor='black')
...
# model.fit(t_in[:int(N_one/2)], y_one[:int(N_one/2)])
...
p0 = (4, 4, -np.pi/2, -np.pi/2, np.max(np.abs(y_one)), np.max(np.abs(y_one)))
